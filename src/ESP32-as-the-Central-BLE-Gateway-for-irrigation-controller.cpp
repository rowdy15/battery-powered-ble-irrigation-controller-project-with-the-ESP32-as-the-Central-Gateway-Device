...
#define frontGardenTapGatewayConnection "irrigation/frontGardenTapGateway/Connection"
...
static BLEUUID    tapStateCharacteristicUUID("F3641402-00B0-4240-BA50-05CA45BF8ABC");
...
static BLERemoteCharacteristic* pRemoteTapStateCharacteristic;
...
void setupTapNotifications(void){
   // Obtain a reference to the service we are after in the remote BLE server.
    ...
    // Obtain a reference to the characteristic in the service of the remote BLE server.
    pRemoteTapStateCharacteristic = pRemoteTapService->getCharacteristic(tapStateCharacteristicUUID);
    if (pRemoteTapCharacteristic == nullptr) {
      Serial.print("Failed to find our characteristic UUID: ");
      Serial.println(tapStateCharacteristicUUID.toString().c_str());
      pClient->disconnect();
    }
    Serial.println(" - Found our tap State characteristic");
...
class MyClientCallback : public BLEClientCallbacks {
  ...
  void onDisconnect(BLEClient* pclient) {
    ...
    ble_connected = false;   
    if(mqtt_connected){
      mqttClient.publish(frontGardenTapConnection,0,false,"not connected");
    }
    // feedWDT = 0;
  }
};
...
void enableTapNotifications(void) {
    ...
    Serial.println("The remote tap characteristic has notify and it has been enabled");
  }

    if(pRemoteTapStateCharacteristic->canNotify()) {
    pRemoteTapStateCharacteristic->registerForNotify(notifyCallback);
    Serial.println("The remote tap state characteristic has notify and it has been enabled");
  }
}
...
void disableTapNotifications(void) {
    ...
    pRemoteTapStateCharacteristic->registerForNotify(NULL);
...
static void notifyCallback( BLERemoteCharacteristic* pBLERemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify) {
 ...
 if(pBLERemoteCharacteristic->getUUID().equals(tapCharacteristicUUID)){
   ...
   if(pBLERemoteCharacteristic->getUUID().equals(tapStateCharacteristicUUID)){
    int tapState = static_cast<uint32_t>(*pData);
    if(tapState == 0){
      mqttClient.publish(frontGardenTapState,0,false,"fully closed");
      Serial.println("The tap is fully closed.");
    } else if (tapState == 1){

      mqttClient.publish(frontGardenTapState,0,false, "fully open");
      Serial.println("The tap is fully open.");
    }   
    else if (tapState == 2){

      mqttClient.publish(frontGardenTapState,0,false, "partially open");
      Serial.println("The tap is partially open.");
    }    
    else if (tapState == 3){

      mqttClient.publish(frontGardenTapState,0,false, "status is unknown");
      Serial.println("The tap state is unknown.");
    }     
  }
...
void onMqttConnect(bool sessionPresent) {
  ...
  // delay(5);
  mqttClient.publish(frontGardenTapGatewayConnection,0,false,"connected");  // 

  if(isMQTTConnectOnBoot){
    mqttClient.publish(frontGardenTapConnection,0,false,"not connected");
    isMQTTConnectOnBoot = false;
  }
...
void onMqttMessage(char* topic, char* payload, AsyncMqttClientMessageProperties properties, size_t len, size_t index, size_t total) {
  ...
  Serial.println(s.substring(0,24));
    if (s == "off"){ 
      ...
      if(s != NULL){  // check to make sure the value is not null
        if(s.toInt() > 0 && s.toInt() < 8) // makes sure the nrf52832 doesn't error too.
